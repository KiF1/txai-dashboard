...
interface ShowModalCreateProps{
  showModal: () => void;
}
...
export function ModalCreate({ showModal }: ShowModalCreateProps){
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: CreateProductForm) => {
      ...
      await api.post('/products', updatedData);
    },
    onSuccess: async () => {
      reset();
      showModal();
    ...
    <div className="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <form onSubmit={handleSubmit((data: CreateProductForm) => mutate(data))} className="relative transform overflow-hidden rounded-lg bg-gray-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <div className="bg-gray-800 px-12 pt-12 pb-6 flex flex-col gap-4">
              <div className="w-full flex justify-between items-center border-b-2 pb-2 border-b-gray-300">
                <strong className="text-xl font-semibold text-black">Cadastrar novo produto</strong>
              ...
              <div className="grid grid-cols-[1fr,0.2fr] gap-4">
                ...
                <div className="w-full flex flex-col gap-2">
                  ...
                  <div className="flex items-center justify-between gap-2 h-[37px] border-2 border-green-600 rounded-[3px] p-2">
                    <button type="button" disabled={quantityProduct === 0} onClick={() => setQuantityProduct(quantityProduct => quantityProduct -= 1)} className="bg-transparent border-0">
                      <MinusIcon className="w-4 h-4" color="#2D7575" />
                    </button>
                    <span className="text-sm font-normal text-black">{quantityProduct}</span>
                    <button onClick={() => setQuantityProduct(quantityProduct => quantityProduct += 1)} type="button" className="bg-transparent border-0">
            ...
            <div className="bg-gray-800 px-12 pb-12 flex justify-end items-center">
              ...
              <button type="submit" disabled={isPending} className='w-[100px] p-2 bg-green-600 text-white font-sm font-medium rounded disabled:cursor-not-allowed disabled:opacity-50'>Cadastrar</button>
            </div>
          </form>
        </div>
      </div>
