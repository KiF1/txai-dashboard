...
export function ModalDeleteUser({ idUser, showModal }: ShowModalDeleteProps){
  const [errorMessage, setErrorMessage] = useState(false);
  
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      await api.delete(`/accounts/${idUser}`);
    },
    onSuccess: async () => {
      showModal();
        ...
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <div className="relative transform overflow-hidden rounded-lg bg-gray-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <div className="bg-gray-800 px-4 pt-4 pb-12 flex flex-col gap-4">
              ...
              {errorMessage && <span className="text-sm font-normal text-red-600">Erro ao deletar usuário, tente novamente mais tarde!</span>}
            </div>
